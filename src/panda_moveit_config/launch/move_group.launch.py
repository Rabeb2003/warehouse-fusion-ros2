...
def generate_launch_description():
    ...
    robot_state_publisher_name = LaunchConfiguration("robot_state_publisher_name")
    ...
    gazebo_grasp_plugin = LaunchConfiguration("gazebo_grasp_plugin")
    ...
    nodes = [
        ...
        Node(
            ...
            output="log",
            arguments=["--ros-args", "--log-level", log_level, "--log-level", "robot_state_publisher:=fatal"],
            parameters=[
                robot_description,
                {
        ...
        Node(
            ...
            output="log",
            arguments=["--ros-args", "--log-level", log_level, "--log-level", "tf2_buffer:=fatal"],
            parameters=[
                robot_description,
                robot_description_semantic,
                kinematics,
        ...
        # rviz2 (delayed by 8 seconds to let move_group fully initialize parameter server)
        TimerAction(
            period=8.0,
            actions=[
                Node(
                    ...
                    parameters=[
                        ...
                        planning_pipeline,
                        joint_limits,
                        {"use_sim_time": use_sim_time, "move_group_namespace": ""},
                    ],
    ...
    # Load and activate all controllers from a single spawner. Parallel spawners
    # can race on switch_controller while Ignition is still settling the model.
    controller_names = ["joint_state_broadcaster"] + moveit_controller_manager_yaml[
        "controller_names"
    ]
    nodes.append(
        Node(
            package="controller_manager",
            executable="spawner",
            name="spawner_panda_controllers",
            output="log",
...
def generate_declared_arguments() -> List[DeclareLaunchArgument]:
    ...
    return [
        ...
        DeclareLaunchArgument(
            "robot_state_publisher_name",
            default_value="panda_robot_state_publisher",
            description="Unique robot_state_publisher node name used by ign_ros2_control.",
        ),
        DeclareLaunchArgument(
            "robot_description_topic",
            default_value="/panda/robot_description",
            description="Topic used by Gazebo create to receive the robot description.",
        ),
        DeclareLaunchArgument(
            "origin_xyz",
            default_value="0 0 0",
            description="World-frame origin of the robot base.",
        ),
        DeclareLaunchArgument(
            "origin_rpy",
            default_value="0 0 0",
            description="World-frame orientation of the robot base.",
        ),
        # Gripper
        DeclareLaunchArgument(
            "gripper",
        ...
        DeclareLaunchArgument(
            "gazebo_grasp_plugin",
            default_value="false",
            description="Flag to enable Gazebo DetachableJoint grasp helper for the pick and place world.",
        ),
        # Servo
        DeclareLaunchArgument(
            "enable_servo",
